...
bool Result::operator<(Result const& right) const
{
	...
	return find_intersection_point(this->result.bers, this->result.fers, line) < find_intersection_point(right.result.bers, right.result.fers, line);
}
...
Result compute_average_result(std::vector<Result> const& results)
{
	...
	for (size_t i{0}; i < results.size() - 1; ++i) {
		if (results[i].result.bers.size() != results[i + 1].result.bers.size()) {
			throw std::runtime_error{"Incompatible results"};
		}
